...
import com.dyw.register.entity.FaceInfoEntity;
import com.dyw.register.handler.FaceSendHandler;
...
public class FaceService {
    ...
    public Boolean setFaceInfo(String cardNo, byte[] byteFace, NativeLong lUserID) {
        try {
            ...
            lpInBuffer.write();
            // 启动远程配置
            NativeLong lHandle = EgciController.hcNetSDK.NET_DVR_StartRemoteConfig(lUserID, HCNetSDK.NET_DVR_SET_FACE_PARAM_CFG,
            ...
            pSendBuf.write();
            boolean result = EgciController.hcNetSDK.NET_DVR_SendRemoteConfig(lHandle, HCNetSDK.ENUM_ACS_INTELLIGENT_IDENTITY_DATA,
    ...
    public Boolean delFace(String cardNo, NativeLong lUserID) {
        try {
            ...
            boolean lRemoteCtrl = EgciController.hcNetSDK.NET_DVR_RemoteControl(lUserID, HCNetSDK.NET_DVR_DEL_FACE_PARAM_CFG, lpInBuffer, m_struFaceDel.size());
            Thread.sleep(500);
            if (!lRemoteCtrl) {
